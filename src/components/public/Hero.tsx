...
export default function Hero({ lang, settings }: Props) {
  ...
  useEffect(() => {
    ...
    if (!el) return
    const obs = new IntersectionObserver(([e]) => e.isIntersecting && el.classList.add('visible'), { threshold: 0.1 })
    obs.observe(el)
      ...
      <div className="container" style={{ position: 'relative', zIndex: 1 }}>
        ...
        >
          ...
          <div>
            ...
            >
              <span style={{ display: 'block', color: 'var(--fg)' }}>{c.h1a}</span>
              <span
                style={{
                  display: 'block',
                  color: 'transparent',
              ...
              <span style={{ display: 'block', color: 'var(--fg)' }}>{c.h1c}</span>
            </h1>
          ...
          >
            {metaValues.map((val, i) => (
              <div
                key={i}
                style={{ display: 'flex', justifyContent: 'space-between', gap: 20, color: 'var(--fg-dim)' }}
              >
